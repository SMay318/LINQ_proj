...
namespace DatabaseFirstLINQ
{
    class Problems
    {
        ...
        private void ProblemThree()
        {
            ...
            var oneHundredFiftyDollars = products.Where(p => p.Price > 150);
            foreach (Product product in oneHundredFiftyDollars)
            {
        ...
        private void ProblemFour()
        {
            ...
            var products = _context.Products;
            var productsWithSLetter = products.Where(p => p.Name.Contains("s"));
            foreach ( Product product in productsWithSLetter)
            {
        ...
        private void ProblemSix()
        {
            ...
            // Then print each user's email and registration date to the console.
            var users = _context.Users;
            var usersBeforeDate = users.Where(u => u.RegistrationDate.Value.Year >= 2016 &&  u.RegistrationDate.Value.Year <= 2018);
            foreach (User user in usersBeforeDate)
            {
                Console.WriteLine($"Email:{user.Email} Registration Date:{user.RegistrationDate}");
            }
        ...
        private void ProblemEight()
        {
            ...
            var userShoppingCart = _context.ShoppingCarts.Include(u => u.Product).Include(u => u.User).Where(u => u.User.Email == "afton@gmail.com");
            foreach (ShoppingCart shoppingCart in userShoppingCart)
            {
        ...
        private void ProblemNine()
        {
            ...
            var userShoppingCart = _context.ShoppingCarts.Include(sc => sc.Product).Include(sc => sc.User).Where(sc => sc.User.Email == "oda@gmail.com").Select(sc => sc.Product.Price).Sum();
        ...
        private void ProblemSixteen()
        {
            ...
            var product = _context.Products.Where(p => p.Name == "Red Ryder BB-Gun").SingleOrDefault();
            product.Price = 25;
...
<<<<<<< HEAD
            //var shoppingCartTotal = _context.ShoppingCarts.Include(sc => sc.Product).Include(sc => sc.User).Include(ut => ut.Quantity).Where(sc => sc.User.Email == "afton@gmail.com").Select(sc => sc.Product.Price * sc.Quantity).Sum();
            decimal? total = 0;
            foreach(User user in users)
            {
                ...
                var shoppingCartQuantity = _context.ShoppingCarts.Include(sh => sh.User).Where(sh => sh.User.Email == user.Email).Select(sh => sh.Quantity).Sum();
                total += userTotals * shoppingCartQuantity ;
                Console.WriteLine($"{user.Email}: ${(userTotals) *(shoppingCartQuantity)}");
=======
            //var shoppingCartTotal = _context.ShoppingCarts.Include(sc => sc.Product).Include(sc => sc.User).Where(sc => sc.User.Email == "afton@gmail.com").Select(sc => sc.Product.Price).Sum();
            decimal total = 0;
            foreach(User user in users)
            {
                decimal userTotals = _context.ShoppingCarts.Include(ut => ut.Product).Include(ut => ut.User).Where(ut => ut.User.Email == user.Email).Select(ut => ut.Product.Price).Sum();
...
>>>>>>> d162259d1563db9163f0e67097a581e27adf8832
            }
